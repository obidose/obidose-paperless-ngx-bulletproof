...
# Back-compat shim: map `log` to `say` if missing
...
# ---------- config state (safe defaults; secrets filled later) ----------
...
ensure_runtime_defaults(){
  [ -n "${PAPERLESS_ADMIN_PASSWORD:-}" ] || PAPERLESS_ADMIN_PASSWORD="$(randpass 2>/dev/null || echo "Admin$(date +%s)")"
  [ -n "${POSTGRES_PASSWORD:-}" ] || POSTGRES_PASSWORD="$(randpass 2>/dev/null || echo "Pg$(date +%s)")"
}
